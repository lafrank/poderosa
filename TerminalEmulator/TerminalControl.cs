...
namespace Poderosa.Terminal
{
	...
	public class TerminalControl : CharacterDocumentViewer
	{
		...
		private ToolStripMenuItem tsmDisconnect;
		private ToolStripMenuItem tsmRestartSession;
		private ToolStripMenuItem tsmNewSession;
		private ToolStripMenuItem tsmDuplicateSession;
		...
		public event EventHandler OnDuplicateSession;
		public event EventHandler OnRestartSession;
		...
		public TerminalControl()
		{
			...
			tsmDuplicateSession.Click += tsmDuplicateSession_Click;
			tsmNewSession.Click += tsmNewSession_Click;
			tsmDisconnect.Click += tsmDisconnect_Click;
			tsmRestartSession.Click += tsmRestartSession_Click;
		...
		private void InitializeComponent()
		{
			...
			this._sizeTip = new System.Windows.Forms.Label();
			this.contextMenu = new System.Windows.Forms.ContextMenuStrip(this.components);
			this.tsmDisconnect = new System.Windows.Forms.ToolStripMenuItem();
			this.tsmRestartSession = new System.Windows.Forms.ToolStripMenuItem();
			this.tsmNewSession = new System.Windows.Forms.ToolStripMenuItem();
			this.tsmDuplicateSession = new System.Windows.Forms.ToolStripMenuItem();
			...
			// _sizeTip
			// 
			this._sizeTip.BackColor = System.Drawing.SystemColors.Info;
			this._sizeTip.BorderStyle = System.Windows.Forms.BorderStyle.FixedSingle;
			this._sizeTip.ForeColor = System.Drawing.SystemColors.InfoText;
			this._sizeTip.Location = new System.Drawing.Point(0, 0);
			this._sizeTip.Name = "_sizeTip";
			this._sizeTip.Size = new System.Drawing.Size(64, 16);
			this._sizeTip.TabIndex = 1;
			this._sizeTip.TextAlign = System.Drawing.ContentAlignment.MiddleCenter;
            ...
            this.tsmDisconnect,
            this.tsmRestartSession,
            this.tsmNewSession,
            this.tsmDuplicateSession,
            this.toolStripSeparator1,
            this.tsmCopy,
            this.tsmPaste});
			this.contextMenu.Name = "contextMenu";
			this.contextMenu.Size = new System.Drawing.Size(166, 142);
			// 
			// tsmDisconnect
			// 
			this.tsmDisconnect.ImageTransparentColor = System.Drawing.Color.Black;
			this.tsmDisconnect.Name = "tsmDisconnect";
			this.tsmDisconnect.Size = new System.Drawing.Size(165, 22);
			this.tsmDisconnect.Text = "Disconnect";
			// 
			// tsmRestartSession
			// 
			this.tsmRestartSession.Name = "tsmRestartSession";
			this.tsmRestartSession.Size = new System.Drawing.Size(165, 22);
			this.tsmRestartSession.Text = "Restart Session";
			// 
			// tsmNewSession
			// 
			this.tsmNewSession.Name = "tsmNewSession";
			this.tsmNewSession.Size = new System.Drawing.Size(165, 22);
			this.tsmNewSession.Text = "New Session";
			// 
			// tsmDuplicateSession
			// 
			this.tsmDuplicateSession.Name = "tsmDuplicateSession";
			this.tsmDuplicateSession.Size = new System.Drawing.Size(165, 22);
			this.tsmDuplicateSession.Text = "Duplicate session";
			// 
			// toolStripSeparator1
			// 
			...
			// 
			this.AllowDrop = true;
			this.Controls.Add(this._sizeTip);
			this.TabStop = false;
			this.Controls.SetChildIndex(this._sizeTip, 0);
			this.Controls.SetChildIndex(this._VScrollBar, 0);
		...
		protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
		{
			...
			if (IsAcceptableUserInput() && (modifiers & Keys.Alt) != Keys.None)
			{ //Altキーの横取り処理を開始
		...
		protected override bool ProcessDialogKey(Keys key)
		{
			...
			if (IsAcceptableUserInput())
			{
				...
				else if (keybody == Keys.Space && modifiers == Keys.Control)
				{ //これはOnKeyPressにわたってくれない
					SendChar('\0');
					return true;
				}
		...
		private void MakeCurrentLineVisible()
		{
			...
			else if (_VScrollBar.Value + document.TerminalHeight <= document.CurrentLineNumber - document.FirstLineNumber)
			{ //下に隠れた
		...
		{
			if (_session != null)
			{ // TerminalPaneを閉じるタイミングでこのメソッドが呼ばれたときにNullReferenceExceptionになるのを防ぐ
				_ignoreValueChangeEvent = true;
		...
		public void CopySelectedTextToClipboard()
		{
			...
			//if (t.Length > 0) Clipboard.SetDataObject(t, false);
		}

		private void tsmDuplicateSession_Click(object sender, EventArgs e)
		{
			OnDuplicateSession?.Invoke(this, new EventArgs());
		}
		private void tsmNewSession_Click(object sender, EventArgs e)
		{
			OnNewSession?.Invoke(this, new EventArgs());
		}
		...
		{
			tsmDisconnect.Enabled = this._session.TerminalConnection?.Socket != null;
			tsmRestartSession.Enabled = this._session.TerminalConnection?.Socket != null;
		}

		private void tsmRestartSession_Click(object sender, EventArgs e)
		{
			OnRestartSession?.Invoke(this, new EventArgs());
		}
		...
		protected override void WndProc(ref Message msg)
		{
			...
			{
				if ((msg.LParam.ToInt32() & 0xFF) == 0)
				{ //最終確定時の特殊処理へ迂回させるフラグを立てる
